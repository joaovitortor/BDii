...
```
> Quando o campo é um array, `{ generos: "Drama" }` já verifica se "Drama" está contido nele — não precisa de operador especial.
...
**16.** Precisa conter **ambos** ao mesmo tempo → `$all`:
```javascript
...
**18.** Pelo menos um dos gêneros → `$in`:
```javascript
db.conteudos.find({ generos: { $in: ["Terror", "Mistério"] } })
```

**19.** Não contém o gênero → `$nin` (evite `$ne` com array, pois `$ne` também exclui certos casos de igualdade exata do array inteiro; `$nin` é o correto para "não contém nenhum destes"):
```javascript
...
**36.** (o `find` já faz um "E" implícito entre campos, mas dá pra usar `$and` explicitamente também)
```javascript
...
**46.** (removendo o usuário criado no exercício 6)
```javascript
db.usuarios.deleteOne({ email: "pedro.bidoia@email.com" })
```

**47.** (removendo o conteúdo criado no exercício 7)
```javascript
